...
export default function PurchaseScreen() {
  ...
  useEffect(() => {
    if (supplierName && suppliers.length > 0) {
      // Find the master object that matches the passed string name
  ...
  }, [supplierName, suppliers]); // Re-runs when master array finishes loading
  ...
  // Convert Date object into YYYY-MM-DD string format
  ...
  // Cart Modifications Handling Logics
  ...
  const handleDecreaseCartQty = (itemCode: string) => {
    setCart((prev) => {
      ...
      if (currentQty <= 1) {
        return prev; // Force manual trash icon tap to delete completely
      }
  ...
  const mockQRCodeScanner = () => {
    Alert.alert(
      "QR Scanner",
      "QR စနစ် ဖွင့်လှစ်နေပါသည်... (Mock Scanner Active)",
    );
  };
      ...
      <Modal visible={cartModalVisible} transparent animationType="slide">
        ...
        >
          <View style={styles.cartBottomSheetContainer}>
            {/* Modal Pull Bar Indicator & Header */}
